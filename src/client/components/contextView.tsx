...
export function makeContextView(
  ...
  function ContextViewBody(props: SessionStandardProps): ReactNS.ReactElement {
    ...
    // control↔toggle link is visible without remounting the tab (the store's subscribe/getSnapshot idiom).
    const subscribeClip = React.useCallback((fn: () => void) => settings.store.subscribe(fn), [settings])
    const readClip = React.useCallback(() => settings.defaultClipAxis(), [settings])
    const clipAxis = React.useSyncExternalStore(subscribeClip, readClip)
